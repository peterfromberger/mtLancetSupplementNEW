...
module <- "module six (sexuality)"
tbl_cap <- paste0("Ordinal regression model for the Explicit Sexual Interest Questionnaire (ESIQ, subscale child) ", q_name, " with n = ", as.character(nobs(fit1)), " observations. Note, that the dependent variable has 3 ordered categories: worsening (increase), no change, improvement (decrease).")
fig_cap <- paste("Diagnostic plot for a linear regression models with ", q_name, ". The plot shows that the assumptions of a linear model are not met.")
```
{{< pagebreak >}}
...
![Rainplot of the Explicit Sexual Interest Questionnaire (ESIQ, total score). Note, that the dependent variable has 3 ordered categories: worsening (increase), no change, improvement (decrease).](_figures/fig-sec-rainplot-esiq-calc-total-child.png){#fig-sec-rainplot-esiq-calc-total}
...
![Diagnostic plots for the Explicit Sexual Interest Questionnaire (ESIQ, subscore child). Note, that the dependent variable has three ordered categories: worsening (increase), no change, improvement (decrease).](_figures/fig-sec-diagnostic-plots-esiq-calc-total-child.png){#fig-sec-rainplot-esiq-calc-total}
...
# save tbl as image
gt::gtsave(tbl, file = file.path(here::here("_tables/tbl-sec-lm-esiq-calc-total-child.pdf")))
gt::gtsave(tbl, file = file.path(here::here("_tables/tbl-sec-lm-esiq-calc-total-child.docx")))
gt::gtsave(tbl, file = file.path(here::here("_tables/tbl-sec-lm-esiq-calc-total-child.png")))
