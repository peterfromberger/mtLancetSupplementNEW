...
# Intervention
wide_csv_path <- "_data/WBI_CONTENT/CONTENT_INTERVENTION_SUMMARIZED/summaries_wide_gt.csv"
summary_csv_path <- "_data/WBI_CONTENT/CONTENT_INTERVENTION_SUMMARIZED/summaries_wide_summary.csv"
intervention_all <- read_csv(summary_csv_path) %>% filter(is.na(lection_rank) & is.na(session_nr)) %>% select(module_nr, summary)

# Placebo
wide_csv_path <- "_data/WBI_CONTENT/CONTENT_PLACEBO_SUMMARIZED/summaries_wide_gt.csv"
summary_csv_path <- "_data/WBI_CONTENT/CONTENT_PLACEBO_SUMMARIZED/summaries_wide_summary.csv"
placebo_all <- read_csv(summary_csv_path) %>% filter(is.na(lection_rank) & is.na(session_nr)) %>% select(module_nr, summary)

tbl_dat <- intervention_all %>%
  left_join(placebo_all, by = c("module_nr" = "module_nr"))

tbl <- gt(tbl_dat) %>%
  fmt_markdown(columns = vars(everything())) %>%
  cols_label(
    module_nr = "Module Number",
    summary.x = "Intervention",
    summary.y = "Placebo"
  ) %>%
