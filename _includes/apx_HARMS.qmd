...
#| tbl-cap: "Frequencies of adverse events during working on the WBI. For each participant, we identified whether they reported the highest possible score for each one-item self-rport scale of the Mood and Risk Questionnaire (MRQ; 'Often' for items 1–8, 'Strongly agree' for item 9, or 'Very bad' for item 10) at least once during the post-intervention assessment. These binary indicators were then used to calculate frequencies and percentages across treatment groups. The MRQ post questionnaire explicitly asks for the time during working on the WBI, whereas the MRQ pre questionnaire eclusiviely asks for the last week before measurement timepoint. Please note, that the occurence of a report of the highest possible score was defined as adverse event post-hoc."
...
tbl <- mrq_at_least_one_highest_score_group %>%
    ...
    tbl_summary(
        ...
        label = list(
            flashlight_1_max_once ~ "...have you thought about how to best make contact with a child? (Contact planning)",
            flashlight_2_max_once ~ "...have you made preparations to be able to make contact with a child? (Contact preparation)",
            flashlight_3_max_once ~ "...have you felt that you must commit a sexual act with a child? (Urge for CSA)",
            flashlight_4_max_once ~ "...have you felt that you might soon (again) watch child sexual abuse material? (Urge for CSAM)",
            flashlight_5_max_once ~ "...have you felt that sexual tension has built up within you? (Sexual tension)",
            flashlight_6_max_once ~ "...have you felt that it is difficult for you to control your sexual thoughts and activities? (Control of sexual thoughts and activity)",
            flashlight_7_max_once ~ "...have you felt that you can no longer endure burdens? (Unable to cope with the mental burden)",
            flashlight_8_max_once ~ "...have you thought about taking your own life? (Suicidal ideations)",
            flashlight_9_max_once ~ "At present, I feel that my behavior and experience are impaired due to mental problems. (Mental crisis)",
            flashlight_10_max_once ~ "How do you feel at the moment? (Very bad current mood)"
        ),
        missing = "no"
    ) %>%
...
#| tbl-cap: "Frequencies of adverse events between working on sessions of the WBI. Adverse events have been assessed with the The Mood and Risk Questionnaire (MRQ) assess acute changes with regard to emotional well-being (suicidal ideations, mental crisis) and thoughts associated with potential re-offenses.  For each participant, we identified whether they reported the highest possible score on a scale of the Mood and Risk Questionnaire (MRQ; 'Often' for items 1–8, 'Strongly agree' for item 9, or 'Very bad' for item 10) at least once during the post-intervention assessment. These binary indicators were then used to calculate frequencies and percentages across treatment groups. The MRQ pre questionnaire explicitly asks asks for the last week before beginning a new session. Please note, that the occurence of a report of the highest possible score was defined as adverse event post-hoc."
#| html-table-processing: none
...
### Adverse events reported by supervision officers
...
#| tbl-cap: "Frequencies of adverse events as mentioned by the supervision officer at exclusion."
#| html-table-processing: none
...
tmp_wide <- tmp %>%
  select(client_id, treatment, exclusion_reasons_list) %>%
...
tbl <- tmp_wide %>%
    ...
    select(-client_id, -`Violation of probation conditions`, -`Violation of probation conditions, Acute suicide risk`, -`CSEM offense, Acute suicide risk`, -`CSA offense, Other offense (not CSA or CSEM), Acute suicide risk`) %>%
    tbl_summary(
        by = treatment
        #type = everything() ~ "categorical"
    ) %>%
