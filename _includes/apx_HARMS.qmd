...
#| tbl-cap: "Frequencies of adverse events during working on the web-based intervention. The MRQ Post questionnaire explicitly asks for the time during working on one session. Shown are the frequency of participants reported at least once the highest possible score (adverse event) on one of the measurement time-points (after each session of the web-based intervention)."
...
tbl <- mrq_at_least_one_highest_score_group %>%
    ...
    add_p(test = everything() ~ fishers_test) %>%
    add_q(method = "holm") %>%
    modify_fmt_fun(
      estimate ~ function(x) style_number(x, digits = 2, decimal.mark = "·")
    ) %>%
    modify_header(
        label = "**MRQ subscale**",
        estimate = "**Cramer's V**",
...
# save tbl as image
gt::gtsave(tbl_a, file = file.path(here::here("_tables/tbl-mrq-frequency-highest-value-post.pdf")))
gt::gtsave(tbl_a, file = file.path(here::here("_tables/tbl-mrq-frequency-highest-value-post.docx")))
gt::gtsave(tbl_a, file = file.path(here::here("_tables/tbl-mrq-frequency-highest-value-post.png")))
...
#| tbl-cap: "Frequencies of adverse events between working on sessions of the web-based intervention. The MRQ pre questionnaire explicitly asks for the last week before beginning a new session. Adverse events were defined as the occurence of the highest possible score (adverse event) on one of the measurement time-points (after each session of the web-based intervention)."
...
# save tbl as image
gt::gtsave(tbl_a, file = file.path(here::here("_tables/tbl-mrq-frequency-highest-value-pre.pdf")))
gt::gtsave(tbl_a, file = file.path(here::here("_tables/tbl-mrq-frequency-highest-value-pre.docx")))
gt::gtsave(tbl_a, file = file.path(here::here("_tables/tbl-mrq-frequency-highest-value-pre.png")))
...
#| tbl-cap: "Frequencies of exclusion reasons reported by the supervision officers. Exclusion reasons have been assessed by the Exclusion Questionnaire (EQ)."
